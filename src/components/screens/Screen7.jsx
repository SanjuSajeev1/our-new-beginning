...
export default function Screen7({ onNext, onBack, isFirst, isLast, direction }) {
  ...
  return (
    <>
      ...
      <motion.section
        ...
        transition={{ opacity: { duration: 0.8, ease: 'easeOut' }, x: { duration: 0.8, ease: 'easeOut' }, scale: { duration: 5, ease: 'linear' } }}
      >
        <motion.div
        ...
        <div className="screen7-text-wrap">
          {lines.map((line, index) => (
            <motion.p
              ...
              transition={{ duration: 0.7, delay: index * 0.52, ease: 'easeOut' }}
            >
              {line}
            </motion.p>
          ))}
